...
static void handleFlagTransferred(Player *fromTank, Player *toTank,
                                  int flagIndex);
...
static const char *blowedUpMessage[] = {NULL,
                                        "Got shot by ",
                                        "Got flattened by ",
                                        ...
                                        "Tank Rusted"};
static bool gotBlowedUp(BaseLocalPlayer *tank, BlowedUpReason reason,
                        PlayerId killer, const ShotPath *hit = NULL,
                        int physicsDriver = -1);
...
std::vector<std::string> &getSilenceList() { return silencePlayers; }
...
void selectNextRecipient(bool forward, bool robotIn) {
    ...
    while (true) {
        ...
        if (i == rindex)
            break;
        if (remotePlayers[i] &&
            (robotIn || remotePlayers[i]->getPlayerType() == TankPlayer)) {
...
static void setGrabMouse(bool grab) { grabMouseAlways = grab; }
...
void warnAboutMainFlags() {
    ...
    if (!BZDBCache::displayMainFlags) {
        ...
        std::vector<std::string> keys =
            KEYMGR.getKeysFromCommand("toggleFlags main", true);
...
void warnAboutRadarFlags() {
    if (!BZDB.isTrue("displayRadarFlags")) {
        ...
        std::vector<std::string> keys =
            KEYMGR.getKeysFromCommand("toggleFlags radar", true);
...
void warnAboutRadar() {
    if (!BZDB.isTrue("displayRadar")) {
        ...
        std::vector<std::string> keys =
            KEYMGR.getKeysFromCommand("toggleRadar", true);
...
void warnAboutConsole() {
    if (!BZDB.isTrue("displayConsole")) {
        ...
        std::vector<std::string> keys =
            KEYMGR.getKeysFromCommand("toggleConsole", true);
...
inline bool isViewTank(Player *tank) {
    return (((tank != NULL) && (tank == LocalPlayer::getMyTank())) ||
            (ROAM.isRoaming() && (ROAM.getMode() == Roaming::roamViewFP) &&
             (ROAM.getTargetTank() == tank)));
}

BzfDisplay *getDisplay() { return display; }

MainWindow *getMainWindow() { return mainWindow; }

ShotStats *getShotStats() { return shotStats; }
...
StartupInfo *getStartupInfo() { return &startupInfo; }
...
static bool doKeyCommon(const BzfKeyEvent &key, bool pressed) {
    ...
    if (key.ascii == 27) {
        ...
        return true;
    } else if (scoreboard->getHuntState() ==
               ScoreboardRenderer::HUNT_SELECTING) {
...
static void doKeyPlaying(const BzfKeyEvent &key, bool pressed,
    ...
    if (key.ascii == '`' && pressed && !haveBinding && key.shift) {
        ...
        if (motionFreeze)
            addMessage(NULL, "The tank's motion is now frozen! ... Press "
                             "Shift+` to unfreeze");
        return;
    }
#endif

    if (key.ascii == 0 && key.button >= BzfKeyEvent::F1 &&
        key.button <= BzfKeyEvent::F10 &&
        (key.shift & (BzfKeyEvent::ControlKey + BzfKeyEvent::AltKey)) != 0 &&
        !haveBinding) {
        ...
        if (pressed) {
            ...
            if (BZDB.isSet(name)) {
                ...
                strncpy(messageBuffer, BZDB.get(name).c_str(), MessageLen - 1);
                messageBuffer[MessageLen - 1] = '\0';
                nboPackString(buf, messageBuffer, MessageLen);
                serverLink->send(MsgMessage, sizeof(messageMessage),
                                 messageMessage);
            }
        }
...
void applyJSModifiers(float &jsx, float &jsy) {
    // enforce range setting limits (min 0% to 20%, max 25% to 100%) so the
    // maximum can't be less than the minimum
    const auto jsRangeMax = std::max(
        std::min(float(BZDB.evalInt("jsRangeMax")) / 100.0f, 1.0f), 0.25f);
    const auto jsRangeMin = std::max(
        std::min(float(BZDB.evalInt("jsRangeMin")) / 100.0f, 0.2f), 0.0f);
    ...
    const auto jsRangeMultiplier = jsRangeMax * (jsMagnitude - jsRangeMin) /
                                   (jsRangeMax - jsRangeMin) / jsMagnitude;
    ...
    if (BZDB.isTrue("jsStretchCorners")) {
        const auto stretchFactor =
            (1.0f -
             float(std::abs(std::abs(atan(jsy / jsx) / M_PI) - 0.25f)) * 4.0f) *
            jsMagnitude;
        ...
        if (!isnan(stretchFactor)) {
            jsx *=
                (1.0f - stretchFactor) +
                stretchValue *
                    stretchFactor; // mix based on stretchFactor from 0.0 to 1.0
    ...
    if (jsxAbs > jsRangeMax || jsyAbs > jsRangeMax) {
        const auto jsRangeExcess =
            (jsxAbs > jsyAbs ? jsxAbs : jsyAbs) / jsRangeMax;
        jsx /= jsRangeExcess;
        jsy /= jsRangeExcess;
    }
}
...
static void doMotion() {
    ...
    // can lead to stuttering movement with X and software rendering (uncertain
    // why)
    int mx = 0, my = 0;
    mainWindow->getMousePosition(mx, my);

    // determine if joystick motion should be used instead of mouse motion
    // when the player bumps the mouse, LocalPlayer::getInputMethod return
    // Mouse; make it return Joystick when the user bumps the joystick
    auto jsx = 0.0f, jsy = 0.0f;
    if (mainWindow->haveJoystick()) {
        if (myTank->getInputMethod() == LocalPlayer::Joystick) {
            // if we're using the joystick right now, replace mouse coords with
            // joystick coords
            mainWindow->getJoyPosition(jsx, jsy);
        } else {
            // if the joystick is not active, and we're not forced to some other
            // input method, see if it's moved and autoswitch
            if (BZDB.isTrue("allowInputChange")) {
                mainWindow->getJoyPosition(jsx, jsy);

                // if we aren't using the joystick, but it's moving, start using
                // it
                if (std::sqrt(jsx * jsx + jsy * jsy) >
                    std::min(
                        std::max(float(BZDB.evalInt("jsRangeMin")) / 100.0f,
                                 0.0f),
                        0.2f))
                    myTank->setInputMethod(
                        LocalPlayer::Joystick); // joystick motion
            }                                   // allowInputChange
    ...
    } else // mouse
    {
        ...
        if (keyboardRotation && !devDriving) {
            ...
            rotation =
                float(-mx - noMotionSize) / float(maxMotionSize - noMotionSize);
            if (rotation > 1.0f)
                rotation = 1.0f;
        } else if (mx > noMotionSize) {
            rotation =
        ...
        if (keyboardSpeed && !devDriving) {
            ...
            speed =
                float(-my - noMotionSize) / float(maxMotionSize - noMotionSize);
            if (speed > 1.0f)
                speed = 1.0f;
        } else if (my > noMotionSize) {
            speed =
...
static void mouseClamp() {
    ...
    if (HUDDialogStack::get()->isActive() || (myTank == NULL) ||
        !myTank->isAlive() || myTank->isPaused() ||
    ...
    const float effScale =
        scale * (0.7f + RENDERER.getMaxMotionFactor() / 16.667f);
...
static void doEvent(BzfDisplay *disply) {
    ...
    case BzfEvent::Resize:
        if (mainWindow->getWidth() != event.resize.width ||
            ...
            mainWindow->getWindow()->setSize(event.resize.width,
                                             event.resize.height);
            mainWindow->getWindow()->callResizeCallbacks();
        }
        break;
    ...
    case BzfEvent::Unmap:
        ...
        // already paused.
        if (!pausedByUnmap && (pauseCountdown == 0.0f) && myTank &&
            myTank->isAlive() && !myTank->isPaused() &&
            !myTank->isAutoPilot() &&
        ...
        if (mainWindow->getFullscreen() &&
            !unmapped) // skip if already unmapped to avoid losing previous
                       // resolution
        {
            preUnmapFormat = -1;
...
void addMessage(const Player *_player, const std::string &msg, int mode,
                bool highlight, const char *oldColor) {
...
static void updateHighScores() {
    ...
    if (myTank && Team::isColorTeam(myTank->getTeam())) {
        ...
        for (i = 0; i < NumTeams; i++) {
            ...
            if (team.size > 0 &&
                team.getWins() - team.getLosses() >= bestScore) {
                haveBest = false;
                break;
            }
        }
...
static void updateFlag(FlagType *flag) {
    if (flag == Flags::Null) {
        ...
        hud->setAlert(2, flag->flagName.c_str(), 3.0f,
    ...
    hud->setAltitudeTape(flag == Flags::Jumping ||
...
void notifyBzfKeyMapChanged() {
    ...
    if (keys.empty()) {
        ...
        if (keys.empty())
            std::cerr
                << "There does not appear to be any key bound to enter the game"
                << std::endl;
    }
...
static Player *addPlayer(PlayerId id, const void *msg, int showMessage) {
    ...
    if (i < 0) {
        ...
        std::cerr << "WARNING: invalid player identification when adding "
                     "player with id "
                  << i << std::endl;
        return NULL;
    }
    ...
    if (remotePlayers[i]) {
        // we're not in synch with server -> help! not a good sign, but not
        // fatal.
        printError("Server error when adding player, player already added");
        std::cerr << "WARNING: player already exists at location with id " << i
                  << std::endl;
        return NULL;
    }
    ...
    if (PlayerType(type) == ComputerPlayer)
        for (int j = 0; j < numRobots; j++)
            if (robots[j] &&
                !strncmp(robots[j]->getCallSign(), callsign, CallSignLen)) {
...
static bool removePlayer(PlayerId id) {
    ...
    while ((playerIndex >= 0) && (playerIndex + 1 == curMaxPlayers) &&
           (remotePlayers[playerIndex] == NULL)) {
        playerIndex--;
        curMaxPlayers--;
    }
...
int curlProgressFunc(void *UNUSED(clientp), curl_off_t dltotal,
                     curl_off_t dlnow, curl_off_t UNUSED(ultotal),
...
static void loadCachedWorld() {
    ...
    std::istream *cachedWorld =
        FILEMGR.createDataInStream(worldCachePath, true);
    if (!cachedWorld) {
        HUDDialogStack::get()->setFailedMessage(
    ...
    if (!localWorldDatabase) {
        HUDDialogStack::get()->setFailedMessage(
            "Error loading cached world.  Join canceled");
        drawFrame(0.0f);
        remove(worldCachePath.c_str());
        joiningGame = false;
        return;
...
static void dumpMissingFlag(const char *buf, uint16_t len) {
    ...
    HUDDialogStack::get()->setFailedMessage(
        TextUtils::format("Flags not supported by this client: {1}", &args)
            .c_str());
}
...
static void handleNearFlag(const void *msg, uint16_t) {
    ...
    addMessage(NULL,
               ColorStrings[YellowColor] + fullMessage +
                   ColorStrings[DefaultColor],
               2, false, NULL);
...
static void handleServerMessage(bool human, uint16_t code, uint16_t len,
                                const void *msg) {
    ...
    case MsgFetchResources:
        if (BZDB.isSet("_noRemoteFiles") && BZDB.isTrue("_noRemoteFiles"))
            ...
            for (int i = 0; i < numItems; i++) {
                ...
                item.filePath =
                    PlatformFactory::getMedia()->getMediaDirectory() +
                    DirectorySeparator;
    ...
    case MsgAutoPilot: {
        ...
        addMessage(tank, autopilot ? "Roger taking controls"
                                   : "Roger releasing controls");
        break;
    }
    ...
    case MsgKilled: {
        ...
        if (victimPlayer == myTank) {
            // uh oh, i'm dead
            if (myTank->isAlive()) {
                const float tmp[3] = {myTank->getPosition()[0].val(),
                                      myTank->getPosition()[1].val(),
                                      myTank->getPosition()[2].val()};
        ...
        if (victimLocal) {
            ...
            const float tmp[3] = {victimPlayer->getPosition()[0].val(),
                                  victimPlayer->getPosition()[1].val(),
                                  victimPlayer->getPosition()[2].val()};
            const float *pos = tmp;
            ...
            EFFECTS.addDeathEffect(victimPlayer->getColor(), pos,
                                   victimPlayer->getAngle(), reason,
                                   victimPlayer, flagType);
            ...
            if (victimPlayer && killerLocal != victimPlayer) {
                if ((victimPlayer->getTeam() == killerLocal->getTeam()) &&
                    ...
                    if (myTank->isAutoPilot()) {
                        if (killerPlayer) {
                            const ShotPath *shot =
                                killerPlayer->getShot(int(shotId));
                            if (shot != NULL)
        ...
        if ((killerPlayer == myTank || victimPlayer == myTank) &&
            ...
            if (killerLocal == myTank) {
                ...
                if (killerPlayer)
                    killerPlayer->changeLocalScore(
                        0, 1,
                        killerPlayer->getTeam() == victimPlayer->getTeam() ? 1
                                                                           : 0);
        ...
        {
            // blow up if killer has genocide flag and i'm on same team as
            // victim (and we're not rogues, unless in rabbit mode)
                ...
                if (shot && shot->getFlag() == Flags::Genocide)
                    for (int i = 0; i < numRobots; i++)
                        if (robots[i] && victimPlayer != robots[i] &&
                            ...
                            gotBlowedUp(robots[i], GenocideEffect,
                                        killerPlayer->getId());
            }
#endif
        }
    ...
    case MsgCaptureFlag: {
        ...
        if (capturer) {
            ...
            if (int(capturer->getTeam()) == capturedTeam) {
                ...
                if (capturer == myTank) {
                    hud->setAlert(1, "Don't capture your own flag!!!", 3.0f,
                                  true);
                    playLocalSound(SFX_KILL_TEAM);
        ...
        for (int i = 0; i < curMaxPlayers; i++) {
            if (remotePlayers[i] && remotePlayers[i]->isAlive() &&
                ...
                const float tmp[3] = {remotePlayers[i]->getPosition()[0].val(),
                                      remotePlayers[i]->getPosition()[1].val(),
                ...
                // todo hook this back up for 2.4.4. or later
                TankDeathOverride *death = NULL;
                EFFECTS.addDeathEffect(remotePlayers[i]->getColor(), pos,
                                       remotePlayers[i]->getAngle(),
                                       GotCaptured, remotePlayers[i], NULL);
    ...
    case MsgNewRabbit: {
        ...
        for (int i = 0; i < curMaxPlayers; i++) {
            ...
            if (i != id && remotePlayers[i] &&
                remotePlayers[i]->getTeam() != HunterTeam &&
        ...
        for (uint8_t s = 0; s < numHandicaps; s++) {
            ...
            if (!sPlayer) {
                ...
                if (i >= 0)
                    ...
                    logDebugMessage(
                        1, "Received handicap update for unknown player!\n");
            }
            if (sPlayer) {
                // a relative score of -_handicapScoreDiff points will provide
                // maximum handicap
                float normalizedHandicap =
                    float(handicap) /
                    std::max(1.0f,
        ...
        for (uint8_t s = 0; s < numScores; s++) {
            ...
            if (id == myTank->getId())
                ...
                if (i >= 0)
                    ...
                    logDebugMessage(
                        1, "Received score update for unknown player!\n");
            }
            if (sPlayer)
    ...
    case MsgTeleport: {
        ...
        if (tank && tank != myTank) {
            ...
            if (teleporter) {
                ...
                tank->setTeleport(TimeKeeper::getTick(), short(from),
                                  short(to));
    ...
    case MsgMessage: {
        ...
        if (MessageType(type) != ChatMessage &&
            MessageType(type) != ActionMessage)
            break;
        ...
        const std::string srcName =
            fromServer ? "SERVER"
                       : (srcPlayer ? srcPlayer->getCallSign() : "(UNKNOWN)");
        if (dstPlayer)
        ...
        if (wordfilter != NULL) {
            ...
            if (wordfilter->filter(filtered))
                msg = filtered;
            origText = stripAnsiCodes(std::string(
                (const char *)msg)); // use filtered[] while it is in scope
        } else
            origText = stripAnsiCodes(std::string((const char *)msg));

        std::string text =
            BundleMgr::getCurrentBundle()->getLocalString(origText);
        ...
        if (toAll || toAdmin || srcPlayer == myTank || dstPlayer == myTank ||
            ...
            if (dstPlayer) {
                // if (fromServer && (origText == "You are now an
                // administrator!"
                //              || origText == "Password Accepted, welcome
                //              back."))
                // admin = true;
                ...
                if (dstPlayer == myTank && srcPlayer == myTank)
                    ...
                    if (srcPlayer == myTank) {
                        ...
                        // play a sound on a private message not from self or
                        // server

                        bool playSound = !fromServer;
                        if (BZDB.isSet("beepOnServerMsg") &&
                            BZDB.isTrue("beepOnServerMsg"))
                            playSound = true;

                        if (playSound) {
                            static TimeKeeper lastMsg =
                                TimeKeeper::getSunGenesisTime();
...
static void handlePlayerMessage(uint16_t code, uint16_t, const void *msg) {
    switch (code) {
    case MsgPlayerUpdate:
    case MsgPlayerUpdateSmall: {
        float timestamp; // could be used to enhance deadreckoning, but isn't
                         // for now
        PlayerId id;
        int32_t order;
        const void *buf = msg;
...
bool addExplosion(const float *_pos, float size, float duration,
                  bool grounded) {
    ...
    // turn up the volume
    newExplosion->setLightColor(1.0f * lightGain, 0.8f * lightGain,
                                0.5f * lightGain);
    while (boom--) {
        // pick a random prototype explosion
...
static void handleFlagTransferred(Player *fromTank, Player *toTank,
    ...
    const float tmp[3] = {toTank->getPosition()[0].val(),
                          toTank->getPosition()[1].val(),
                          toTank->getPosition()[2].val()};
    const float *pos = tmp;
...
static bool gotBlowedUp(BaseLocalPlayer *tank, BlowedUpReason reason,
                        PlayerId killer, const ShotPath *hit, int phydrv) {
    ...
    if (reason != GotShot || flag != Flags::Shield) {
        ...
        const float tmp[3] = {tank->getPosition()[0].val(),
                              tank->getPosition()[1].val(),
                              tank->getPosition()[2].val()};
        EFFECTS.addDeathEffect(tank->getColor(), tmp, tank->getAngle(), reason,
                               tank, flagType);
        ...
        if (reason == GotShot || reason == GotRunOver ||
            ...
            lookupServer(tank)->sendKilled(killer, reason, shotId, flagType,
                                           phydrv);
    }
    ...
    if ((tank == myTank) &&
        ...
        if (reason == GotShot && getLocalPlayer(killer) == myTank)
            ...
            // 1-4 are messages sent when the player dies because of someone
            // else
            if (reason >= GotShot && reason <= GenocideEffect) {
                ...
                if (!killerPlayer)
                    ...
                    if (hit)
                        team = hit->getTeam();
                    if (World::getWorld()->allowTeams() &&
                        (myTank->getTeam() == team) && (team != RogueTeam) &&
                        (team != ObserverTeam)) {
                        ...
                        blowedUpNotice += " (";
                        blowedUpNotice +=
                            Team::getName(killerPlayer->getTeam());
                        blowedUpNotice += ")";
...
static void checkEnvironment() {
    ...
    if (myTank->getTeam() == ObserverTeam) {
        ...
        if ((flagd == Flags::Narrow) || (flagd == Flags::Tiny))
            // Don't bother trying to figure this out with a narrow or tiny flag
            // yet.
            return;
        ...
        smsg << "local collision with " << hit->getShotId() << " from "
             << hitter->getCallSign() << std::endl;
    ...
    if (flagd->flagTeam != NoTeam) {
        ...
        if ((base != NoTeam) && ((flagd->flagTeam == team && base != team) ||
            ...
            serverLink->sendCaptureFlag(base);
               ...
               (myTank->getLocation() == LocalPlayer::OnGround ||
    ...
    else if ((waterLevel > 0.0f) &&
             (myTank->getPosition()[2].val() <= waterLevel))
    ...
    // if not dead yet, see if i got run over by the steamroller
        ...
        for (i = 0; i < curMaxPlayers; i++) {
            if (remotePlayers[i] && !remotePlayers[i]->isPaused() &&
                ...
                if (pos[2] < 0.0f)
                    continue;
                if (remotePlayers[i]->getTeam() != RogueTeam &&
                    !World::getWorld()->allowTeamKills() &&
                    remotePlayers[i]->getTeam() == myTank->getTeam())
                    continue;
                if (!myTank->isPhantomZoned()) {
                    const float radius =
                        myRadius + BZDB.eval(StateDatabase::BZDB_SRRADIUSMULT) *
                                       remotePlayers[i]->getRadius();
                    const float distSquared =
                        hypotf(hypotf(myPos[0] - pos[0], myPos[1] - pos[1]),
                               (myPos[2] - pos[2]) * 2.0f);
                    if (distSquared < radius)
                        gotBlowedUp(myTank, GotRunOver,
                                    remotePlayers[i]->getId());
                }
            }
        }
    }
...
bool inLookRange(float angle, float distance, float bestDistance,
                 RemotePlayer *player) {
    // usually about 17 degrees
    if (angle >= BZDB.eval(StateDatabase::BZDB_TARGETINGANGLE))
        return false;

    if (distance > BZDB.eval(StateDatabase::BZDB_TARGETINGDISTANCE) ||
...
void setLookAtMarker(void) {
    ...
    for (int i = 0; i < curMaxPlayers; i++) {
        ...
        const fvec3 rPos(remotePlayers[i]->getPosition()[0].val(),
                         remotePlayers[i]->getPosition()[1].val(),
                         remotePlayers[i]->getPosition()[2].val());
    ...
    const float tmp2[3] = {bestTarget->getPosition()[0].val(),
                           bestTarget->getPosition()[1].val(),
                           bestTarget->getPosition()[2].val()};
...
void setTarget() {
    ...
    for (int i = 0; i < curMaxPlayers; i++) {
        ...
        if (a < BZDB.eval(
                    StateDatabase::BZDB_LOCKONANGLE) &&     // about 8.5 degrees
            ((myTank->getFlag() == Flags::GuidedMissile) || // am i locking on?
             tankHasShotType(myTank, Flags::GuidedMissile)) &&
            remotePlayers[i]->getFlag() !=
                Flags::Stealth &&            // can't lock on stealth
            !remotePlayers[i]->isPaused() && // can't lock on paused
            !remotePlayers[i]
                 ->isNotResponding() && // can't lock on not responding
            d < bestDistance)           // is it better?
        {
            ...
            lockedOn = true;
                           ...
                           StateDatabase::BZDB_TARGETINGANGLE) && // about 17
                                                                  // degrees
                   ((remotePlayers[i]->getFlag() != Flags::Stealth) ||
                    (myTank->getFlag() ==
                     Flags::Seer)) && // can't "see" stealth unless have seer
                   d < bestDistance &&
                   !lockedOn) // is it better?
        {
            bestTarget = remotePlayers[i];
            bestDistance = d;
        }
...
static void setHuntTarget() {
    ...
    if (bestTarget->isHunted() && myTank->getFlag() != Flags::Blindness &&
        ...
        if (!pulse.isOn()) {
            const float tmp[3] = {bestTarget->getPosition()[0].val(),
                                  bestTarget->getPosition()[1].val(),
                                  bestTarget->getPosition()[2].val()};
...
static void addObstacle(std::vector<BzfRegion *> &rgnList,
...
static void setRobotTarget(RobotPlayer *robot) {
    ...
    for (int j = 0; j < curMaxPlayers; j++)
        if (remotePlayers[j] && remotePlayers[j]->getId() != robot->getId() &&
            ...
            robot->validTeamTarget(remotePlayers[j])) {

            if (remotePlayers[j]->isPhantomZoned() && !robot->isPhantomZoned())
                continue;

            if (World::getWorld()->allowTeamFlags() &&
                ((robot->getTeam() == RedTeam &&
                  remotePlayers[j]->getFlag() == Flags::RedTeam) ||
                 (robot->getTeam() == GreenTeam &&
                  remotePlayers[j]->getFlag() == Flags::GreenTeam) ||
                 (robot->getTeam() == BlueTeam &&
                  remotePlayers[j]->getFlag() == Flags::BlueTeam) ||
                 (robot->getTeam() == PurpleTeam &&
                  remotePlayers[j]->getFlag() == Flags::PurpleTeam))) {
...
static void checkEnvironment(RobotPlayer *tank) {
    ...
    if (hit) {
        ...
        // after dropping our shield flag.
        if (hit->isStoppedByHit())
            lookupServer(tank)->sendEndShot(hit->getPlayer(), hit->getShotId(),
                                            1);
    ...
    // if not dead yet, see if the robot dropped below the death level
             ...
             (tank->getPosition()[2].val() <= waterLevel))
...
static void addRobots() {
    ...
    for (i = 0, j = 0; i < numRobotTanks; i++) {
        robotServer[j] =
            new ServerLink(serverNetworkAddress, startupInfo.serverPort);
        if (!robotServer[j] || robotServer[j]->getState() != ServerLink::Okay) {
            delete robotServer[j];
            continue;
        } else {
            snprintf(callsign, CallSignLen, "%.29s%2.2x", myTank->getCallSign(),
                     (short int)j);
    ...
    for (j = 0; j < numRobots; j++) {
        // wait for response
        if (robotServer[j] && (robotServer[j]->read(code, len, msg, -1) < 0 ||
                               code != MsgAccept)) {
            delete robots[j];
            delete robotServer[j];
            robots[j] = NULL;
...
static void setTankFlags() {
    ...
    for (int i = 0; i < maxFlags; i++) {
        ...
        if (flag.status == FlagOnTank) {
            for (int j = 0; j < curMaxPlayers; j++) {
                if (remotePlayers[j] &&
                    remotePlayers[j]->getId() == flag.owner) {
...
static void enteringServer(const void *buf) {
    ...
    if (myTank->getTeam() != AutomaticTeam) {
        ...
        if ((TeamColor)team == ObserverTeam)
            ...
            teamMsg +=
                TextUtils::format("to the %s", Team::getName((TeamColor)team));
        }
    } else {
        if ((TeamColor)team == ObserverTeam)
    ...
    while (true) {
        ...
        if (h != INVALID_HANDLE_VALUE) {
            ...
            while (FindNextFile(h, &findData)) {
                if ((oldestFile == NULL) ||
                    (CompareFileTime(&oldestTime, &findData.ftLastAccessTime) >
                     0)) {
                    if (oldestFile)
                        free(oldestFile);
...
static void markOld(std::string &fileName) {
    ...
    FILETIME ft;
    HANDLE h =
        CreateFile(fileName.c_str(), FILE_WRITE_ATTRIBUTES | FILE_WRITE_EA, 0,
...
static void sendFlagNegotiation() {
     ...
     */
    for (i = FlagType::getFlagMap().begin(); i != FlagType::getFlagMap().end();
         ++i)
...
static void saveRobotInfo(Playerid id, void *msg) {
    for (int i = 0; i < numRobots; i++)
        if (robots[i]) {
            ...
            tmpbuf = nboUnpackString(tmpbuf, motto, MottoLen);
            std::cerr << "id " << id.port << ':' << id.number << ':' << callsign
                      << ' ' << robots[i]->getId().port << ':'
            ...
            if (strncmp(robots[i]->getCallSign(), callsign, CallSignLen)) {
                ...
                snprintf(buffer, 100, "id test %p %p %p %8.8x %8.8x\n",
                         robots[i], tmpbuf, msg, *(int *)tmpbuf,
                         *((int *)tmpbuf + 1));
                std::cerr << buffer;
...
void leaveGame() {
    ...
    */
    ...
    if (sayGoodbye)
        serverLink->send(MsgExit, 0, NULL);
    ...
    sceneRenderer->getViewFrustum().setProjection(
        (float)(60.0 * M_PI / 180.0), NearPlaneNormal, FarPlaneDefault,
        FarDeepPlaneDefault, mainWindow->getWidth(), mainWindow->getHeight(),
...
static void joinInternetGame() {
    ...
    if (serverLink->getState() != ServerLink::Okay) {
        ...
        case ServerLink::BadVersion: {
            ...
            snprintf(versionError, sizeof(versionError),
                     "Incompatible server version %s",
                     serverLink->getVersion());
...
static void addVarToAutoComplete(const std::string &name,
...
static void joinInternetGame2() {
    ...
    serverLink->sendEnter(TankPlayer, myTank->getTeam(), myTank->getCallSign(),
                          myTank->getMotto(), startupInfo.token);
    startupInfo.token[0] = '\0';
...
static bool trackPlayerShot(Player *target, float *eyePoint,
                            float *targetPoint) {
    ...
    if (BZDB.isTrue("trackShots")) {
        ...
        if (sp != NULL) {
            ...
            if (speed > 0.0f) {
                ...
                const float dir[3] = {ilen * vel[0], ilen * vel[1],
                                      ilen * vel[2]};
                ...
                eyePoint[0] =
                    pos[0] + (offset * dir[0]) + (tOffset * topDir[0]);
                eyePoint[1] =
                    pos[1] + (offset * dir[1]) + (tOffset * topDir[1]);
                eyePoint[2] =
                    pos[2] + (offset * dir[2]) + (tOffset * topDir[2]);
...
void drawFrame(const float dt) {
    ...
    if (!unmapped) {
        ...
        if (!myTank) {
            ...
            // float tmp[3] = { myTank->getPosition()[0].val(),
            ...
            // myTankPos = { myTank->getPosition()[0].val(),
            // myTank->getPosition()[1].val(), myTank->getPosition()[2].val()};
            // printf("camera pos3: %f, %f, %f\n", myTankPos[0], myTankPos[1],
            // myTankPos[2]);
        ...
        if (devDriving || ROAM.isRoaming()) {
            ...
            if (!(ROAM.getMode() == Roaming::roamViewFree) &&
                    ...
                    if (!trackPlayerShot(target, eyePoint, targetPoint)) {
                        ...
                        if (slowKB == (BZDB.eval("roamSmoothTime") < 0.0f)) {
                            ...
                            eyePoint[1] = target->getPosition()[1].val() -
                                          targetTankDir[1] * 40;
                            eyePoint[2] = target->getPosition()[2].val() +
                                          muzzleHeight * 6;
                            targetPoint[0] = target->getPosition()[0].val();
                            targetPoint[1] = target->getPosition()[1].val();
                            targetPoint[2] = target->getPosition()[2].val();
                ...
                roamViewAngle = (float)(atan2(targetPoint[1] - eyePoint[1],
                                              targetPoint[0] - eyePoint[0]) *
                                        180.0f / M_PI);
            }
            // free Roaming
            else {
                float dir[3];
                dir[0] = cosf((float)(roam->phi * M_PI / 180.0)) *
                         cosf((float)(roam->theta * M_PI / 180.0));
                dir[1] = cosf((float)(roam->phi * M_PI / 180.0)) *
                         sinf((float)(roam->theta * M_PI / 180.0));
            ...
            if (!devDriving) {
                ...
                if (myTank)
                    myTank->move(virtPos,
                                 (float)(roamViewAngle * M_PI / 180.0));
            }
            fov = (float)(roam->zoom * M_PI / 180.0);
            moveSoundReceiver(eyePoint[0], eyePoint[1], eyePoint[2],
                              myTankAngle, false);
        }
        ...
        viewFrustum.setProjection(
            fov, NearPlane, FarPlane, FarDeepPlane, mainWindow->getWidth(),
        ...
        if (scene && myTank) {
            ...
            const bool showMyTreads =
                showTreads ||
                (devDriving && (ROAM.getMode() != Roaming::roamViewFP));
            myTank->addToScene(scene, myTank->getTeam(), inCockpit, seerView,
        ...
        sceneRenderer->setBlank(
            myTank &&
            (myTank->isPaused() || myTank->getFlag() == Flags::Blindness));
        ...
        if (myTank) {
            ...
            const Obstacle *obs;
            obs =
                world->inBuilding(clipPos, myTank->getAngle(), hnp, 0.0f, 0.0f);
            if (obs != NULL)
                insideDim = true;
        }
        ...
        hud->setCracks(myTank && !firstLife && !justJoined &&
                       !myTank->isAlive());
        ...
        if (viewType == SceneRenderer::ThreeChannel) {
            ...
            targetPoint[0] =
                eyePoint[0] + cFOV * myTankDir[0] - sFOV * myTankDir[1];
            targetPoint[1] =
                eyePoint[1] + cFOV * myTankDir[1] + sFOV * myTankDir[0];
            targetPoint[2] = eyePoint[2] + myTankDir[2];
            viewFrustum.setView(eyePoint, targetPoint);
            ...
            bool useMultisampling =
                maxSamples > 1 && BZDB.evalInt("multisample") > 1;
            if (useMultisampling) {
                int samples = BZDB.evalInt("multisample");
                if (samples > maxSamples)
                    samples = maxSamples;
                glFramebuffer.checkState(mainWindow->getWidth(),
                                         mainWindow->getHeight(), samples);
            ...
            // blit the multisample framebuffer (if enabled) to the main
            // framebuffer
            if (useMultisampling) {
                ...
                glBlitFramebuffer(
                    0, 0, mainWindow->getWidth(), mainWindow->getHeight(), 0, 0,
...
static void roamSmoothFollow(Roaming::RoamingCamera &deltaCamera) {
    ...
    const float tmp[3] = {p->getPosition()[0].val(), p->getPosition()[1].val(),
                          p->getPosition()[2].val()};
    const float *pos = tmp;
    const float *fwd = p->getForward();
    const float target[3] = {pos[0] - (fwd[0] * dist), pos[1] - (fwd[1] * dist),
                             pos[2] + height};
    const float *current = ROAM.getCamera()->pos;
    const float delta[3] = {target[0] - current[0], target[1] - current[1],
                            target[2] - current[2]};
...
static void setupRoamingCamera(float dt) {
    ...
    if (myTank) {
        ...
        if (currMouseBits != 0) {
            ...
            if (!control && !shift)
                deltaCamera.pos[0] =
                    (float)(4 * myTank->getSpeed()) * BZDBCache::tankSpeed;
            if (alt)
                deltaCamera.pos[1] =
                    (float)(4 * myTank->getRotation()) * BZDBCache::tankSpeed;
            else
                deltaCamera.theta =
                    ROAM.getZoom() * (float)myTank->getRotation();
            if (control)
                deltaCamera.phi =
                    2.0f * ROAM.getZoom() / 3.0f * (float)myTank->getSpeed();
            if (shift)
                deltaCamera.pos[2] =
                    (float)(4 * myTank->getSpeed()) * BZDBCache::tankSpeed;
        }
    ...
    if (BZDB.isTrue("slowKeyboard") != (st < 0.0f)) {
        ...
        deltaCamera.pos[0] =
            (at * deltaCamera.pos[0]) + (bt * prevDeltaCamera.pos[0]);
        deltaCamera.pos[1] =
            (at * deltaCamera.pos[1]) + (bt * prevDeltaCamera.pos[1]);
        deltaCamera.pos[2] =
            (at * deltaCamera.pos[2]) + (bt * prevDeltaCamera.pos[2]);
        deltaCamera.theta =
            (at * deltaCamera.theta) + (bt * prevDeltaCamera.theta);
...
static void prepareTheHUD() {
    ...
    if (myTank) {
        ...
        if (world->allowTeamFlags()) {
            ...
            for (int i = 0; i < numFlags; i++) {
                ...
                if ((flag.type->flagTeam == myTank->getTeam()) &&
                    ...
                    float heading =
                        atan2f(flagPos[1] - myPos[1], flagPos[0] - myPos[0]);
                    hud->addMarker(heading, myTeamColor);
                    hud->AddEnhancedMarker(Float3ToVec3(flagPos),
                                           Float3ToVec4(myTeamColor), false,
        ...
        if (myTank->getAntidoteLocation()) {
            ...
            float heading =
                atan2f(antidotePos[1] - myPos[1], antidotePos[0] - myPos[0]);
            const float antidoteColor[] = {1.0f, 1.0f, 0.0f, 1.0f};
            hud->addMarker(heading, antidoteColor);
            hud->AddEnhancedMarker(Float3ToVec3(antidotePos),
                                   Float4ToVec4(antidoteColor), false,
                                   BZDBCache::flagPoleSize * 2.0f);
        }
    }
    return;
    ...
    if (pauseCountdown > 0.0f) {
        ...
        if (pauseCountdown <= 0.0f) {
            ...
            if (myTank->getLocation() == LocalPlayer::InBuilding) {
                ...
                hud->setAlert(1, "Can't pause while inside a building", 1.0f,
                              false);

            } else if (myTank->getLocation() == LocalPlayer::InAir) {
                // custom message when trying to pause when jumping/falling
                hud->setAlert(1, "Can't pause when you are in the air", 1.0f,
                              false);
                ...
                hud->setAlert(1, "Can't pause when you are in the phantom zone",
                              1.0f, false);
...
static void updateDestructCountdown(float dt) {
    ...
    if (destructCountdown > 0.0f) {
        ...
        if (destructCountdown <= 0.0f) {
            ...
            // update countdown alert
            char msgBuf[40];
            sprintf(msgBuf, "Self Destructing in %d",
                    (int)(destructCountdown + 0.99f));
            hud->setAlert(1, msgBuf, 1.0f, false);
        }
    }
    return;
...
static void playingLoop() {
    ...
    while (!CommandsStandard::isQuit()) {
        ...
        if (waitingDNS) {
            ...
            AresHandler::ResolutionStatus status =
                ares->getHostAddress(&inAddress);
        ...
        if (mainWindow->haveJoystick()) {
            static const BzfKeyEvent::Button button_map[] = {
                BzfKeyEvent::BZ_Button_1,  BzfKeyEvent::BZ_Button_2,
                BzfKeyEvent::BZ_Button_3,  BzfKeyEvent::BZ_Button_4,
                BzfKeyEvent::BZ_Button_5,  BzfKeyEvent::BZ_Button_6,
                BzfKeyEvent::BZ_Button_7,  BzfKeyEvent::BZ_Button_8,
                BzfKeyEvent::BZ_Button_9,  BzfKeyEvent::BZ_Button_10,
                BzfKeyEvent::BZ_Button_11, BzfKeyEvent::BZ_Button_12,
                BzfKeyEvent::BZ_Button_13, BzfKeyEvent::BZ_Button_14,
                BzfKeyEvent::BZ_Button_15, BzfKeyEvent::BZ_Button_16,
                BzfKeyEvent::BZ_Button_17, BzfKeyEvent::BZ_Button_18,
                BzfKeyEvent::BZ_Button_19, BzfKeyEvent::BZ_Button_20,
                BzfKeyEvent::BZ_Button_21, BzfKeyEvent::BZ_Button_22,
                BzfKeyEvent::BZ_Button_23, BzfKeyEvent::BZ_Button_24,
                BzfKeyEvent::BZ_Button_25, BzfKeyEvent::BZ_Button_26,
                BzfKeyEvent::BZ_Button_27, BzfKeyEvent::BZ_Button_28,
                BzfKeyEvent::BZ_Button_29, BzfKeyEvent::BZ_Button_30,
                BzfKeyEvent::BZ_Button_31, BzfKeyEvent::BZ_Button_32,
            };
            ...
            if (num_hats > max_hats)
                num_hats = max_hats; // num_hats min= max_hats;
            ...
            for (int hat = 0; hat < num_hats; hat++) {
                ...
                if (hatX == 0 && hatY == 0) {
                    ...
                    for (int b = -1; b < num_buttons; b++) {
                        float testangle =
                            -180 + 2 * variance * (b + 1); // -180 to 180 by 45
                        if (testangle - variance <= angle &&
                            angle < testangle + variance) {
                            button = b; // 0 to 7
                            if (b == -1)
                                button = num_buttons - 1; // 7
                            if (button != hats[hat]) {
                                if (hats[hat] != -1)
        ...
        if ((myTank != NULL) && (myTank->queryInputChange() == true)) {
            controlPanel->addMessage(
                LocalPlayer::getInputMethodName(myTank->getInputMethod()) +
                " movement");
        }
        ...
        for (i = 0; i < curMaxPlayers; i++) {
            if (remotePlayers[i]) {
                const bool wasNotResponding =
                    remotePlayers[i]->isNotResponding();
                remotePlayers[i]->doDeadReckoning();
                const bool isNotResponding =
                    remotePlayers[i]->isNotResponding();
        ...
        if (myTank) {
            if (myTank->isAlive() && !myTank->isPaused()) {
                doMotion();
                if (scoreboard->getHuntState() ==
                    ScoreboardRenderer::HUNT_ENABLED) {
                ...
                if (targetdPlayer && targetdPlayer->isAlive() &&
                    targetdPlayer->getFlag() != Flags::Stealth) {

                    const float tmp[3] = {
                        myTank->getTarget()->getPosition()[0].val(),
                        myTank->getTarget()->getPosition()[1].val(),
                        myTank->getTarget()->getPosition()[2].val()};
                    hud->AddLockOnMarker(Float3ToVec3(tmp),
                                         myTank->getTarget()->getCallSign(),
                                         !isKillable(myTank->getTarget()));
                } else // if we should not have a target, force that target to
                       // be cleared
                    myTank->setTarget(NULL);
        ...
        if (myTank && myTank->getTeam() == ObserverTeam) {
            if (BZDB.isTrue("sendObserverHeartbeat")) {
                double heartbeatTime = BZDB.isSet("observerHeartbeat")
                                           ? BZDB.eval("observerHeartbeat")
                                           : 30.0f;
                if (lastObserverUpdateTime + heartbeatTime <
                    TimeKeeper::getTick().getSeconds()) {
        ...
        if (BZDB.evalInt("saveEnergy") == 1) {
            ...
            while (1) {
                ...
                if (serverLink && !serverError) {
                    int e = 0;
                    e = serverLink->read(code, len, msg,
                                         int(remaining * 1000.0f));
                    if (e == 1)
                        handleServerMessage(true, code, len, msg);
                    if (e == -2) {
                        printError("Server communication error");
...
void startPlaying(BzfDisplay *_display, SceneRenderer &renderer) {
    ...
    for (unsigned int c = 0; c < bzcountof(commandList); ++c)
        CMDMGR.add(commandList[c].name, commandList[c].func,
                   commandList[c].help);
...
// should we grab the mouse?
    ...
    setGrabMouse(false);
#elif defined(                                                                 \
    __linux__) // linux usually has a virtual root window so grab mouse always
    setGrabMouse(true);
#else
...
#pragma warning(disable : 4996)
    if (debugLevel >= 1) {
        ...
        GetVersionEx(&info);
        logDebugMessage(
            1, "Running on Windows %s%d.%d %s\n",
            (info.dwPlatformId == VER_PLATFORM_WIN32_NT) ? "NT " : "",
            info.dwMajorVersion, info.dwMinorVersion, info.szCSDVersion);
    }
#endif
    ...
    // pop up main menu
    if (startupInfo.autoConnect && startupInfo.callsign[0] &&
        ...
        HUDDialogStack::get()
            ->top()
            ->execute(); // show "Trying Automatic Connection..."
